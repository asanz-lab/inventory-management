...
export const api = {
  ...
  async getRestockRecommendations(budget) {
    const response = await axios.get(`${API_BASE_URL}/restock/recommend`, {
      params: { budget }
    })
    return response.data
  },

  async placeRestockOrder(items, warehouse = null) {
    const response = await axios.post(`${API_BASE_URL}/restock/orders`, {
      items,
      warehouse
    })
    return response.data
